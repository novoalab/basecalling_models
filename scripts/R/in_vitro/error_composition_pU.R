...
get_c_freq <- function(df){
  ...
  final_df <- df %>%
    ...
    filter(modification %in% c("Y","UNM")) %>% 
...
ggsave(filename = "cmis_error_VVUVV.pdf",
       plot = last_plot(),
       path = here("~/Library/CloudStorage/Dropbox/Oz-Gregor/4.IVTmod/results_Gregor/plots/epinanoRMS/per_kmer/pseudoU_pos0_CtoU_mismatch"),
       width = 6,
       height = 4,
       units = "in")
...
ggsave(filename = "VVUVV_kmer_sup.pdf",
       plot = last_plot(),
       path = here("results_Gregor","plots","epinanoRMS", "per_position", "mismatch_dir","VVUVV_kmers"),
       width = 8,
       height = 2,
       units = "in")
